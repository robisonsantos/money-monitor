<script lang="ts">
  ...
  let { title, value, change, changePercent, icon = "dollar", showChange = true }: Props = $props();
...
<div class="card relative">
  ...
  <div class="absolute top-3 right-3">
    <div class="w-10 h-10 bg-primary-100 dark:bg-primary-900/20 rounded-lg flex items-center justify-center">
      <IconComponent class="w-5 h-5 text-primary-600 dark:text-primary-400" />
    </div>
  </div>
  ...
  <div class="pr-20">
    ...
    <p class="text-2xl font-bold text-foreground-primary mt-1">
