<script lang="ts">
    ...
    function startCreating() {
        ...
        setTimeout(() => {
            const input = dropdownElement.querySelector(
                "#new-portfolio-input",
            ) as HTMLInputElement;
            input?.focus();
        }, 0);
    ...
    <div class="flex-shrink-0 relative" bind:this={dropdownElement}>
        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            {#if isLoading}
                <div class="animate-pulse">
                    <div
                        class="h-8 lg:h-10 bg-background-tertiary rounded w-64"
                    ></div>
                </div>
            {:else if isAllPortfoliosSelected}
                <button
                    onclick={() =>
                        !disabled && (isDropdownOpen = !isDropdownOpen)}
                    class="group flex items-center gap-3 px-4 py-3 text-2xl lg:text-3xl font-bold text-foreground-primary hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all duration-200 {disabled
                        ...
                        : 'cursor-pointer'} {isDropdownOpen
                        ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                        : ''}"
                    {disabled}
                >
                    <Folder
                        class="w-6 h-6 lg:w-7 lg:h-7 text-blue-600 dark:text-blue-400 flex-shrink-0"
                    />
                    <span
                        class="truncate max-w-[250px] sm:max-w-[400px] lg:max-w-[500px]"
                        >All Portfolios</span
                    >
                    <ChevronDown
            ...
            {:else}
                ...
                >
                    ...
                    <ChevronDown
                        class="w-5 h-5 transition-transform duration-200 {isDropdownOpen
                            ? 'rotate-180'
                            : ''}"
                    />
                </button>
        ...
        <p class="text-foreground-secondary mt-2 ml-0">
            {#if isAllPortfoliosSelected}
                Investment Dashboard - Track all your portfolios' performance
                over time
            {:else if selectedPortfolio}
                Investment Dashboard - Track your portfolio performance over
                time
            {:else}
            ...
            >
                <div class="py-2">
                    ...
                    <div class="px-4 py-2 border-b border-border-secondary">
                        <h3
                            class="text-sm font-semibold text-foreground-primary flex items-center gap-2"
                        >
                    ...
                    {#if showAllOption}
                        <div class="group relative">
                            <div
                                class="flex items-center {isAllPortfoliosSelected
                                    ? ''
                                    : 'hover:bg-background-secondary'}"
                            >
                                <button
                                    onclick={() => selectPortfolio(null)}
                                    class="flex-1 flex items-center px-4 py-3 text-sm cursor-pointer portfolio-item {isAllPortfoliosSelected
                                        ? 'selected'
                                        : ''}"
                                    class:text-foreground-secondary={!isAllPortfoliosSelected}
                                >
                                    <Folder
                                        class="w-4 h-4 mr-3 text-foreground-tertiary"
                                    />
                                    ...
                                    {#if isAllPortfoliosSelected}
                                        <span
                                            class="ml-auto text-blue-600 dark:text-blue-400 font-semibold"
                                            >✓</span
                                        >
                                    {/if}
                                </button>
                        ...
                        <div
                            class="border-t border-border-secondary my-1"
                        ></div>
                    {/if}
                        ...
                        {#each portfolios as portfolio (portfolio.id)}
                            <div class="group relative">
                                ...
                                {:else}
                                    <!-- Portfolio item -->
                                    <div
                                        class="flex items-center {selectedPortfolio?.id ===
                                        portfolio.id
                                            ? ''
                                            : 'hover:bg-background-secondary'}"
                                    >
                                        <button
                                            ...
                                            class="flex-1 flex items-center px-4 py-3 text-sm cursor-pointer portfolio-item {selectedPortfolio?.id ===
                                            portfolio.id
                                                ? 'selected'
                                                : ''}"
                                            class:text-foreground-secondary={selectedPortfolio?.id !==
                                                portfolio.id}
                                        >
                                            <Folder
                                                class="w-4 h-4 mr-3 {selectedPortfolio?.id ===
                                                portfolio.id
                                                    ? 'text-blue-500'
                                            ...
                                            {#if selectedPortfolio?.id === portfolio.id}
                                                <span
                                                    class="ml-auto text-blue-600 font-semibold"
                                                    >✓</span
                                                >
                                            {/if}
                                        </button>
                                        ...
                                        <!-- Action buttons -->
                                        <div
                                            class="flex items-center pr-2 opacity-0 group-hover:opacity-100 transition-opacity"
                                        >
                                            <button
                                                onclick={(e) => {
                                                    e.stopPropagation();
...
<style>
    ...
    button.portfolio-item.selected {
        background-color: rgb(
            ...
            246,
            255
        ) !important; /* Light blue for light mode */
    ...
    :global(.dark) button.portfolio-item.selected {
        background-color: rgb(
            ...
            65,
            85
        ) !important; /* Dark gray for dark mode */
        color: rgb(
            147,
            197,
            253
        ) !important; /* Light blue text for dark mode */
    }
