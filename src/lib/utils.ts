...
function filterByPeriod(data: AggregatedData[], period: AggregationPeriod, filter: FilterPeriod): AggregatedData[] {
  ...
  switch (period) {
    case "daily":
      aggregated = sortedInvestments.map((investment, index) => {
        ...
        return {
          date: typeof investment.date === "string" ? investment.date : investment.date.toISOString().split("T")[0],
    ...
    case "monthly":
      aggregated = aggregateByPeriod(sortedInvestments, (date) => {
        const dateStr = typeof date === "string" ? date : date.toISOString().split("T")[0];
        return format(startOfMonth(parseISO(dateStr)), "yyyy-MM-dd");
      });
      break;
...
function aggregateByPeriod(investments: Investment[], getPeriodKey: (date: string | Date) => string): AggregatedData[] {
  const periodMap = new Map<string, Investment[]>();

  // Group investments by period
  investments.forEach((investment) => {
    const key = getPeriodKey(investment.date);
    if (!periodMap.has(key)) {
      periodMap.set(key, []);
    }
    periodMap.get(key)!.push(investment);
  });
...
export function generateCSV(investments: { date: string; value: number }[]): string {
  ...
  const rows = investments
    .map((inv) => `${sanitizeCSVValue(inv.date)},${sanitizeCSVValue(inv.value.toString())}`)
    .join("\n");
  return header + rows;
}
