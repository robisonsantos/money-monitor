<script lang="ts">
    ...
    async function loadExistingData(editDate: string) {
        ...
        try {
            ...
            const apiUrl = currentPortfolio
                ? `/api/portfolios/${currentPortfolio.id}/investments`
                : "/api/investments";
    ...
    async function handleSubmit(event?: SubmitEvent) {
        ...
        if (!isEditMode) {
            ...
            if (carryOverWeekend && isFriday()) {
                const saturdayExists = await checkDateExists(
                    weekendDates().saturday,
                );
                const sundayExists = await checkDateExists(
                    weekendDates().sunday,
                );
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 409) {
                    ...
                    throw new Error(
                        errorData.error ||
                            `Failed to ${isEditMode ? "update" : "save"} investment`,
                    );
                }
                return;
            }
...
>
    <div class="max-w-2xl mx-auto">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-lg text-foreground-secondary max-w-md mx-auto">
                Record your portfolio value for a specific date
            </p>
        </div>

        <!-- Enhanced main form card -->
        <div
            class="bg-background-primary rounded-2xl shadow-xl dark:shadow-none dark:ring-1 dark:ring-white/10 dark:bg-gradient-to-br dark:from-slate-800/50 dark:to-slate-900/50 border border-border-primary overflow-hidden mb-8"
        >
            ...
            <div class="p-8 sm:p-10">
                {#if isEditMode}
                    ...
                    >
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 bg-blue-100 dark:bg-blue-800/30 rounded-lg flex items-center justify-center mr-4"
                            >
                                <Edit3
                            ...
                            <div>
                                <p class="text-blue-900 font-semibold text-lg">
                                    Editing existing entry for {formatDate(
                                        date,
                                    )}
                                </p>
                                <p class="text-blue-700 text-sm mt-1">
                                    You can update the portfolio value below.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                {#if success}
                    <div
                        class="bg-success-50 border border-success-200 rounded-xl p-6 mb-8"
                    >
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 bg-success-100 rounded-lg flex items-center justify-center mr-4"
                            >
                                <div class="w-6 h-6 text-success-600 font-bold">
                                    ✓
                                </div>
                            </div>
                            <div>
                                <p
                                    class="text-success-900 font-semibold text-lg"
                                >
                                    Investment entry {isEditMode
                                        ? "updated"
                                        : "saved"} successfully!
                                </p>
                ...
                {#if error}
                    <div
                        class="bg-danger-50 border border-danger-200 rounded-xl p-6 mb-8"
                    >
                        <div class="flex items-center">
                            <div
                                class="w-10 h-10 bg-danger-100 rounded-lg flex items-center justify-center mr-4"
                            >
                                <AlertCircle class="w-5 h-5 text-danger-600" />
                            </div>
                            <div>
                                <p
                                    class="text-danger-900 font-semibold text-lg"
                                >
                                    {error}
                                </p>
                            </div>
                ...
                >
                    ...
                    <div>
                        <label
                            class="block text-sm font-semibold text-foreground-primary mb-3"
                        >
                        ...
                        {#if $portfolioIsLoading}
                            <div class="animate-pulse">
                                <div
                                    class="h-12 bg-background-tertiary rounded-xl"
                                ></div>
                            </div>
                        {:else if $portfolios.length > 0}
                            <PortfolioSelector
                                ...
                                on:rename={(e) =>
                                    portfolioStore.renamePortfolio(
                                        e.detail.portfolio,
                                        e.detail.newName,
                                    )}
                                on:delete={(e) =>
                                    portfolioStore.deletePortfolio(e.detail)}
                            />
                            {#if $portfolioError}
                                <p
                                    class="text-sm text-danger-600 dark:text-danger-400 mt-2"
                                >
                        ...
                        {:else}
                            ...
                            >
                                <p
                                    class="text-warning-800 dark:text-warning-300 text-sm"
                                >
                                    <AlertCircle class="w-4 h-4 inline mr-1" />
                        ...
                        <p class="text-sm text-foreground-tertiary mt-3">
                            Select the portfolio to add this investment entry to
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        {#if dateWarning}
                            <p
                                class="text-sm text-warning-600 dark:text-warning-400 mt-3 font-medium flex items-center"
                            >
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                <span
                                    class="text-foreground-tertiary text-lg font-medium"
                                    >$</span
                                >
                            </div>
                            <input
                        ...
                        >
                            <div class="flex items-start">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p
                                        class="text-sm text-blue-700 dark:text-blue-200 mb-3"
                                    >
                                        Since markets are closed on weekends,
                                        this will also save the same value for:
                                    </p>
                                    <div
                                        class="flex flex-col sm:flex-row gap-2 text-sm"
                                    >
                                        <span
                    ...
                    {#if !isEditMode && dateWarning}
                        ...
                        >
                            <p
                                class="text-sm text-warning-800 dark:text-warning-300 flex items-center"
                            >
                                <AlertCircle
        ...
        <!-- Enhanced Quick Tips -->
        <div
            class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6 dark:bg-gradient-to-br dark:from-slate-800/40 dark:to-slate-900/40 dark:ring-1 dark:ring-white/10"
        >
            <div class="flex items-start">
                <div
                    class="w-6 h-6 text-blue-600 dark:text-blue-400 mr-3 mt-0.5"
                >
                    💡
                </div>
                <div>
